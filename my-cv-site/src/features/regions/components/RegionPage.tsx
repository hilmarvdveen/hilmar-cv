    ...
    <>
      <Section background="white" aria-labelledby="region-pitch-heading">
        <Container width="narrow">
          <SectionTitle id="region-pitch-heading" title={t("shared.pitchTitle")} />
          <Card variant="quiet">
            <p className="text-xl font-semibold leading-snug text-textMain">{home("hiring.pitch.sentence")}</p>
            <p className="mt-3 text-base leading-relaxed text-gray-600">{home("hiring.pitch.examples")}</p>
          </Card>
        </Container>
      </Section>
      ...
      <Section background="white" aria-labelledby="region-travel-heading">
        <Container width="narrow">
          <SectionTitle id="region-travel-heading" title={t("shared.travelTitle", { city })} />
          <Card>
            <ul className="space-y-3">
              {travel.map((item) => (
                <li key={item} className="flex items-start gap-3 text-base leading-relaxed text-gray-700">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                {shapes.map((shape) => (
                  <Card key={shape.title} className="flex h-full flex-col">
                    ...
                    <p className="mt-2 text-base leading-relaxed text-gray-600">{shape.body}</p>
                  </Card>
                ))}
              </div>
            )}
      ...
      <Section background="white" aria-labelledby="region-questions-heading">
        <Container width="narrow">
          <SectionTitle id="region-questions-heading" title={t("shared.questionsTitle", { city })} />
      ...
      <Section background="light" padding="compact" aria-labelledby="region-others-heading">
        <Container width="narrow">
          <h2 id="region-others-heading" className="text-xs font-bold uppercase tracking-widest text-primary">
            {t("shared.othersTitle")}
          </h2>
          <ul className="mt-3 flex flex-wrap gap-x-6 gap-y-2">
            {siblings.map((sibling) => (
              <li key={sibling.id}>
                <Link href={regionPath(sibling)} className={`${linkClass} text-base`} data-placement={`region-${region.id}-sibling`}>
                  {t(`${sibling.id}.name`)}
                </Link>
              </li>
            ))}
            <li>
              <Link href={regionPath()} className={`${linkClass} text-base`}>
                {t("shared.hubLink")}
              </Link>
            </li>
          </ul>
      ...
      <Section background="navy" aria-labelledby="region-close-heading">
        <Container width="narrow" className="text-center">
          <h2 id="region-close-heading" className="mb-4 text-section-title text-balance text-white">
