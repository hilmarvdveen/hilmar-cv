export const WorkMode = {
  Remote: "workMode.remote",
  OnSite: "workMode.onSite",
  Hybrid: "workMode.hybrid",
} as const;

export type WorkMode = (typeof WorkMode)[keyof typeof WorkMode];

export const Language = {
  Dutch: "language.dutch",
  English: "language.english",
} as const;

export type Language = (typeof Language)[keyof typeof Language];

export const Tech = {
  HTML5: "tech.html5",
  CSS3: "tech.css3",
  JavaScript: "tech.javascript",
  TypeScript: "tech.typescript",
  Angular: "tech.angular",
  React: "tech.react",
  Vue: "tech.vue",
  WebComponents: "tech.webComponents",
  Stencil: "tech.stencil",
  WCAG: "tech.wcag",
  WCAG21: "tech.wcag21",
  WCAG22: "tech.wcag22",
  Playwright: "tech.playwright",
  Cypress: "tech.cypress",
  REST: "tech.rest",
  OpenAPI: "tech.openapi",
  JWT: "tech.jwt",
  Java8: "tech.java8",
  Maven: "tech.maven",
  Git: "tech.git",
  NPM: "tech.npm",
  Jenkins: "tech.jenkins",
  Tekton: "tech.tekton",
  ArgoCD: "tech.argocd",
  SPLUNK: "tech.splunk",
  Docker: "tech.docker",
  SonarQube: "tech.sonarqube",
  ESLint: "tech.eslint",
  Storybook: "tech.storybook",
  Confluence: "tech.confluence",
  Jira: "tech.jira",
  MySQL: "tech.mysql",
  CSharp: "tech.csharp",
  Neo4J: "tech.Neo4J",
  Redux: "tech.Redux",
  WPF: "tech.WPF",
  MVVM: "tech.MVVM",
  WebForms: "tech.WebForms",
  Azure: "tech.Azure",
  MSSQL: "tech.MSSQL",
  TailwindCSS: "tech.TailwindCSS",
  NextJS: "tech.NextJS",
  RxJS: "tech.RxJS",
  BEM: "tech.BEM",
  SCSS: "tech.SCSS",
  SVG: "tech.SVG",
  AWSLambda: "tech.AWSLambda",
  GitLabCI: "tech.GitLabCI",
  Prettier: "tech.Prettier",
  HeadlessCMS: "tech.HeadlessCMS",
  ReactQuery: "tech.ReactQuery",
  Zustand: "tech.Zustand",
  Jest: "tech.Jest",
  GitHub: "tech.GitHub",
  ComponentDesign: "tech.ComponentDesign",
  Gitlab: "tech.Gitlab",
  Bitbucket: "tech.Bitbucket",
  DotNetCore: "tech.DotNetCore",
  AzureAppServices: "tech.AzureAppServices",
  AzureCloud: "tech.AzureCloud",
  EntityFramework: "tech.EntityFramework",
  Microservices: "tech.Microservices",
  RestAPI: "tech.RestAPI",
  ConfigurationDrivenUI: "tech.ConfigurationDrivenUI",
  DataDrivenUI: "tech.DataDrivenUI",
  Kubernetes: "tech.Kubernetes",
  xUnit: "tech.xUnit",
  AzureDevOps: "tech.AzureDevOps",
  AngularMaterial: "tech.AngularMaterial",
  AzureCosmosDB: "tech.AzureCosmosDB",
  SQLServer: "tech.SQLServer",
  ReactRouter: "tech.ReactRouter",
  Scrum: "tech.Scrum",
  SourceTree: "tech.SourceTree",
  NHibernate: "tech.NHibernate",
  MediatR: "tech.MediatR",
  AspNetMVC: "tech.AspNetMVC",
  CQRS: "tech.CQRS",
  MediatorPattern: "tech.MediatorPattern",
  OctopusDeploy: "tech.OctopusDeploy",
  UIKit: "tech.UIKit",
  IntegrationTesting: "tech.IntegrationTesting",
  NodeJS: "tech.NodeJS",
  Express: "tech.Express",
  GraphQL: "tech.GraphQL",
  MonoreposNx: "tech.MonoreposNx",
  CleanArchitecture: "tech.CleanArchitecture",
  DomainDrivenDesign: "tech.DomainDrivenDesign",
  EventDrivenDesign: "tech.EventDrivenDesign",
  SSRSSG: "tech.SSRSSG",
  GitHubActions: "tech.GitHubActions",
  CICDPipelines: "tech.CICDPipelines",
  ReactTestingLibrary: "tech.ReactTestingLibrary",
  Vitest: "tech.Vitest",
  PostgreSQL: "tech.PostgreSQL",
  Vercel: "tech.Vercel",
  Netlify: "tech.Netlify",
  Firebase: "tech.Firebase",
  Kotlin: "tech.Kotlin",
  GCP: "tech.GCP",
  Sentry: "tech.Sentry",
  RudderStack: "tech.RudderStack",
  Pnpm: "tech.Pnpm",
  MSW: "tech.MSW",
  Urql: "tech.Urql",
  GqlTada: "tech.GqlTada",
  Terraform: "tech.Terraform",
  Python: "tech.Python",
  OpenIDConnect: "tech.OpenIDConnect",
  SpringBoot: "tech.SpringBoot",
  SpringForGraphQL: "tech.SpringForGraphQL",
  JUnit: "tech.JUnit",
  Mockito: "tech.Mockito",
  Contentstack: "tech.Contentstack",
  Storyblok: "tech.Storyblok",
  Bloomreach: "tech.Bloomreach",
  AspNetCore: "tech.AspNetCore",
  GRPC: "tech.GRPC",
  MicrosoftIdentity: "tech.MicrosoftIdentity",
  AzureKeyVault: "tech.AzureKeyVault",
  MicroFrontends: "tech.MicroFrontends",
  RabbitMQ: "tech.RabbitMQ",
  MassTransit: "tech.MassTransit",
  Linux: "tech.Linux",
  SSH: "tech.SSH",
  MicrosoftGraph: "tech.MicrosoftGraph",
  OAuth2: "tech.OAuth2",
  ContentSecurityPolicy: "tech.ContentSecurityPolicy",
  ApolloServer: "tech.ApolloServer",
  ApolloFederation: "tech.ApolloFederation",
  DotNet10: "tech.DotNet10",
  Java25: "tech.Java25",
  ModelContextProtocolServer: "tech.ModelContextProtocolServer",
  ModelContextProtocolClient: "tech.ModelContextProtocolClient",
  AzureOpenAI: "tech.AzureOpenAI",
  TextToSql: "tech.TextToSql",
  ArtificialIntelligenceAgents: "tech.ArtificialIntelligenceAgents",
  LanguageModelIntegration: "tech.LanguageModelIntegration",
  MongoDB: "tech.MongoDB",
  AzureContainerApps: "tech.AzureContainerApps",
  CodingAgents: "tech.CodingAgents",
} as const;

export type Tech = (typeof Tech)[keyof typeof Tech];

export type WorkEntry = {
  id: string;
  company: string;
  logo: string;
  from: string;
  to: string;
  location: string;
  mode: WorkMode;
  language: Language;
  tech: Tech[];
  color?: string;
}

export const workHistory: WorkEntry[] = [
  {
    id: "bol",
    company: "bol.com",
    logo: "bol.svg",
    from: "2025-07",
    to: "2026-10",
    location: "Utrecht",
    mode: WorkMode.Hybrid,
    language: Language.Dutch,
    tech: [
      Tech.React,
      Tech.TypeScript,
      Tech.ReactRouter,
      Tech.GraphQL,
      Tech.Urql,
      Tech.GqlTada,
      Tech.TailwindCSS,
      Tech.WCAG22,
      Tech.MonoreposNx,
      Tech.Pnpm,
      Tech.Storybook,
      Tech.Vitest,
      Tech.Playwright,
      Tech.MSW,
      Tech.Sentry,
      Tech.RudderStack,
      Tech.Kotlin,
      Tech.GCP,
      Tech.Kubernetes,
      Tech.NodeJS,
      Tech.Express,
      Tech.SpringBoot,
      Tech.SpringForGraphQL,
      Tech.JUnit,
      Tech.Mockito,
      Tech.ReactTestingLibrary,
      Tech.HeadlessCMS,
      Tech.Contentstack,
      Tech.GitLabCI,
      Tech.Docker,
      Tech.CICDPipelines,
      Tech.Linux,
      Tech.SSH,
    ],
  },
  {
    id: "belastingdienst",
    company: "Belastingdienst",
    logo: "belastingdienst-mark.png",
    from: "2023-01",
    to: "2024-12",
    location: "Apeldoorn",
    mode: WorkMode.Hybrid,
    language: Language.Dutch,
    tech: [
      Tech.HTML5,
      Tech.CSS3,
      Tech.JavaScript,
      Tech.TypeScript,
      Tech.Angular,
      Tech.React,
      Tech.Vue,
      Tech.WebComponents,
      Tech.Stencil,
      Tech.WCAG21,
      Tech.Playwright,
      Tech.Cypress,
      Tech.REST,
      Tech.OpenAPI,
      Tech.JWT,
      Tech.Java8,
      Tech.SpringBoot,
      Tech.Maven,
      Tech.Gitlab,
      Tech.Git,
      Tech.NPM,
      Tech.Jenkins,
      Tech.Tekton,
      Tech.ArgoCD,
      Tech.SPLUNK,
      Tech.Docker,
      Tech.Terraform,
      Tech.SonarQube,
      Tech.ESLint,
      Tech.Storybook,
      Tech.Confluence,
      Tech.Jira,
      Tech.MySQL,
      Tech.RxJS,
      Tech.JUnit,
      Tech.Mockito,
      Tech.Azure,
      Tech.CICDPipelines,
      Tech.Linux,
      Tech.SSH,
    ],
  },
  {
    id: "postcode-loterij",
    company: "Nationale Postcode Loterij",
    logo: "nationale-postcode-loterij-mark.png",
    from: "2022-07",
    to: "2023-01",
    location: "Amsterdam",
    mode: WorkMode.Hybrid,
    language: Language.English,
    tech: [
      Tech.HTML5,
      Tech.SCSS,
      Tech.TailwindCSS,
      Tech.JavaScript,
      Tech.TypeScript,
      Tech.React,
      Tech.NextJS,
      Tech.WCAG21,
      Tech.SVG,
      Tech.Cypress,
      Tech.Playwright,
      Tech.OpenAPI,
      Tech.AWSLambda,
      Tech.Python,
      Tech.Git,
      Tech.GitLabCI,
      Tech.Docker,
      Tech.SonarQube,
      Tech.ESLint,
      Tech.Prettier,
      Tech.Jira,
      Tech.Confluence,
      Tech.HeadlessCMS,
      Tech.ReactQuery,
      Tech.Zustand,
      Tech.NodeJS,
      Tech.MonoreposNx,
      Tech.Storyblok,
      Tech.CICDPipelines,
      Tech.Linux,
      Tech.SSH,
    ],
  },
  {
    id: "athlon",
    company: "Athlon",
    logo: "athlon.svg",
    from: "2020-10",
    to: "2022-07",
    location: "Almere",
    mode: WorkMode.Remote,
    language: Language.English,
    tech: [
      Tech.HTML5,
      Tech.SCSS,
      Tech.Angular,
      Tech.RxJS,
      Tech.TypeScript,
      Tech.JavaScript,
      Tech.WCAG21,
      Tech.Jest,
      Tech.Cypress,
      Tech.OpenAPI,
      Tech.REST,
      Tech.Java8,
      Tech.SpringBoot,
      Tech.Azure,
      Tech.GitHub,
      Tech.Docker,
      Tech.Maven,
      Tech.SonarQube,
      Tech.ESLint,
      Tech.Prettier,
      Tech.Jira,
      Tech.Confluence,
      Tech.HeadlessCMS,
      Tech.TailwindCSS,
      Tech.AzureDevOps,
      Tech.Kubernetes,
      Tech.CICDPipelines,
      Tech.Linux,
      Tech.SSH,
    ],
  },
  {
    id: "randstad",
    company: "Randstad",
    logo: "randstad.svg",
    from: "2020-08",
    to: "2020-10",
    location: "Amsterdam",
    mode: WorkMode.OnSite,
    language: Language.English,
    tech: [
      Tech.HTML5,
      Tech.SCSS,
      Tech.JavaScript,
      Tech.Angular,
      Tech.React,
      Tech.BEM,
      Tech.WCAG,
      Tech.ComponentDesign,
      Tech.HeadlessCMS,
      Tech.Maven,
      Tech.Jenkins,
      Tech.Gitlab,
      Tech.Git,
      Tech.GitHub,
      Tech.Bitbucket,
      Tech.SonarQube,
      Tech.Prettier,
      Tech.ESLint,
      Tech.Confluence,
      Tech.Jira,
      Tech.Bloomreach,
      Tech.SSH,
    ],
  },
  {
    id: "opinity",
    company: "Opinity",
    logo: "opinity-mark.png",
    from: "2020-03",
    to: "2020-07",
    location: "Rotterdam",
    mode: WorkMode.OnSite,
    language: Language.English,
    tech: [
      Tech.Angular,
      Tech.TypeScript,
      Tech.RxJS,
      Tech.CSharp,
      Tech.DotNetCore,
      Tech.RestAPI,
      Tech.HTML5,
      Tech.CSS3,
      Tech.ESLint,
      Tech.Prettier,
      Tech.Jira,
      Tech.Git,
      Tech.AspNetCore,
      Tech.SSH,
    ],
  },
  {
    id: "omniplan",
    company: "Omniplan",
    logo: "omniplan.svg",
    from: "2019-08",
    to: "2020-03",
    location: "Amsterdam",
    mode: WorkMode.OnSite,
    language: Language.Dutch,
    tech: [
      Tech.Angular,
      Tech.TypeScript,
      Tech.RxJS,
      Tech.CSharp,
      Tech.DotNetCore,
      Tech.JWT,
      Tech.OpenIDConnect,
      Tech.AzureAppServices,
      Tech.AzureCloud,
      Tech.MSSQL,
      Tech.EntityFramework,
      Tech.Microservices,
      Tech.RestAPI,
      Tech.ConfigurationDrivenUI,
      Tech.DataDrivenUI,
      Tech.Docker,
      Tech.Kubernetes,
      Tech.xUnit,
      Tech.Git,
      Tech.AzureDevOps,
      Tech.Confluence,
      Tech.GRPC,
      Tech.MicrosoftIdentity,
      Tech.AzureKeyVault,
      Tech.Linux,
      Tech.SSH,
    ],
  },
  {
    id: "ortec",
    company: "Ortec",
    logo: "ortec.png",
    from: "2019-04",
    to: "2019-07",
    location: "Zoetermeer",
    mode: WorkMode.OnSite,
    language: Language.English,
    tech: [
      Tech.Angular,
      Tech.AngularMaterial,
      Tech.TypeScript,
      Tech.HTML5,
      Tech.CSS3,
      Tech.RxJS,
      Tech.CSharp,
      Tech.DotNetCore,
      Tech.AzureCloud,
      Tech.AzureCosmosDB,
      Tech.OpenAPI,
      Tech.RestAPI,
      Tech.DataDrivenUI,
      Tech.ConfigurationDrivenUI,
      Tech.Docker,
      Tech.Terraform,
      Tech.SonarQube,
      Tech.Prettier,
      Tech.ESLint,
      Tech.Confluence,
      Tech.Jira,
      Tech.Gitlab,
      Tech.Microservices,
      Tech.MicroFrontends,
      Tech.CICDPipelines,
      Tech.Linux,
      Tech.SSH,
    ],
  },
  {
    id: "bluefield",
    company: "Bluefield Smart Access",
    logo: "bluefield.png",
    from: "2018-03",
    to: "2019-04",
    location: "Utrecht",
    mode: WorkMode.OnSite,
    language: Language.Dutch,
    tech: [
      Tech.React,
      Tech.TypeScript,
      Tech.JavaScript,
      Tech.HTML5,
      Tech.CSS3,
      Tech.Redux,
      Tech.CSharp,
      Tech.DotNetCore,
      Tech.Neo4J,
      Tech.RestAPI,
      Tech.SQLServer,
      Tech.ReactRouter,
      Tech.MVVM,
      Tech.SonarQube,
      Tech.Prettier,
      Tech.ESLint,
      Tech.Confluence,
      Tech.Git,
      Tech.Microservices,
      Tech.EventDrivenDesign,
      Tech.RabbitMQ,
      Tech.MassTransit,
      Tech.WPF,
      Tech.Docker,
      Tech.CICDPipelines,
      Tech.Linux,
      Tech.SSH,
    ],
  },
  {
    id: "niped",
    company: "Niped",
    logo: "niped.svg",
    from: "2017-12",
    to: "2018-03",
    location: "Hoorn",
    mode: WorkMode.OnSite,
    language: Language.English,
    tech: [
      Tech.React,
      Tech.Redux,
      Tech.ReactRouter,
      Tech.JavaScript,
      Tech.HTML5,
      Tech.CSS3,
      Tech.CSharp,
      Tech.WPF,
      Tech.MVVM,
      Tech.RestAPI,
      Tech.SonarQube,
      Tech.Prettier,
      Tech.ESLint,
      Tech.Scrum,
      Tech.SourceTree,
      Tech.Confluence,
      Tech.Bitbucket,
      Tech.Jira,
      Tech.SSH,
    ],
  },
  {
    id: "conclusion",
    company: "Conclusion",
    logo: "conclusion.svg",
    from: "2017-08",
    to: "2017-12",
    location: "Amsterdam",
    mode: WorkMode.OnSite,
    language: Language.Dutch,
    tech: [
      Tech.Vue,
      Tech.JavaScript,
      Tech.HTML5,
      Tech.CSS3,
      Tech.CSharp,
      Tech.WebForms,
      Tech.RestAPI,
      Tech.CQRS,
      Tech.NHibernate,
      Tech.MSSQL,
      Tech.AspNetMVC,
      Tech.SonarQube,
      Tech.xUnit,
      Tech.MediatorPattern,
      Tech.Prettier,
      Tech.Confluence,
      Tech.Bitbucket,
      Tech.Jira,
      Tech.Jenkins,
      Tech.OctopusDeploy,
      Tech.UIKit,
      Tech.SSH,
    ],
  },
  {
    id: "transdev",
    company: "Transdev",
    logo: "transdev.png",
    from: "2016-08",
    to: "2017-08",
    location: "Hilversum",
    mode: WorkMode.OnSite,
    language: Language.English,
    tech: [
      Tech.Angular,
      Tech.TypeScript,
      Tech.RxJS,
      Tech.HTML5,
      Tech.CSS3,
      Tech.JavaScript,
      Tech.CSharp,
      Tech.DotNetCore,
      Tech.AspNetMVC,
      Tech.SQLServer,
      Tech.RestAPI,
      Tech.JWT,
      Tech.SonarQube,
      Tech.Prettier,
      Tech.Jenkins,
      Tech.Confluence,
      Tech.Git,
      Tech.IntegrationTesting,
      Tech.SSH,
    ],
  },
];
